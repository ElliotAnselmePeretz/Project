...
export function Page({ children, width = "md" }: { children: ReactNode; width?: "md" | "lg" }) {
  return (
    <main className={`animate-fade-up mx-auto w-full ${width === "lg" ? "max-w-4xl" : "max-w-2xl"} px-5 py-8 sm:px-6 sm:py-10`}>
      {children}
    </main>
  );
}
...
export function EmptyState({ title, hint }: { title: string; hint?: string }) {
  return (
    <div className="animate-fade-up rounded-lg border border-dashed border-border-strong bg-surface-alt/50 px-6 py-12 text-center transition-colors duration-300 hover:border-accent/40">
